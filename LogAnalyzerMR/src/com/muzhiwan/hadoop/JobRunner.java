...
public class JobRunner {
	...
	static {
		try {
			jobEntryMap.put(TYPE_MOBILE_GAME_DOWNLOAD, JobRunner.class.getMethod("getMobileGameDownloadStatistic", String.class, String.class));
			jobEntryMap.put(TYPE_MOBILE_SDK_ACTIVE_USER, JobRunner.class.getMethod("getMobileSDKActiveUsers", String.class, String.class));
	...
	public static int executeAnalysis(String type, String inputPath,
			String outputPath) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
		
		if (jobEntryMap.containsKey(type)) {
			return (Integer) jobEntryMap.get(type).invoke(null, inputPath, outputPath);
		}
		
		return 0;
	}

	public static int getMobileSDKActiveUsers(String inputPath, String outputPath)
			throws IOException, InterruptedException, ClassNotFoundException {
		long timeStart = System.currentTimeMillis();
		int exitCode;
		
		String analysisPath = String.format("%s/%s/%s", outputPath, TYPE_MOBILE_SDK_ANALYSIS, Utility.timeToDate(timeStart));
		...
		FileOutputFormat.setOutputPath(job, new Path(
				String.format("%s/%s/%s", outputPath, TYPE_MOBILE_SDK_ACTIVE_USER, Utility.timeToDate(timeStart))));
	...
	private static int getMobileSDKAnalysisData(String inputPath, String outputPath)
			throws IOException, InterruptedException, ClassNotFoundException {
		long timeStart = System.currentTimeMillis();
	...
	public static int getMobileGameDownloadStatistic(String inputPath, String outputPath)
			throws IOException, InterruptedException, ClassNotFoundException {
		long timeStart = System.currentTimeMillis();
