...
hive -e "
	...
	insert overwrite table game_active_user_stat 
		select case when CLIENT_TIME>SERVER_TIME*1000 or SERVER_TIME>(CLIENT_TIME/1000)+864000 then from_unixtime(SERVER_TIME,'yyyyMMdd') else from_unixtime(floor(CLIENT_TIME/1000),'yyyyMMdd') end as stat_day,PACKAGE_NAME,VERSION_CODE,count(DISTINCT CELL_PHONE_DEVICE_ID) as user_count from sdk100001 group by case when CLIENT_TIME > SERVER_TIME*1000 or SERVER_TIME>(CLIENT_TIME/1000)+864000 then from_unixtime(SERVER_TIME,'yyyyMMdd') else from_unixtime(floor(CLIENT_TIME/1000),'yyyyMMdd') end,PACKAGE_NAME,VERSION_CODE;
