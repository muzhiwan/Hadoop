...
hive -e "
    ...
    insert overwrite table sdk_new_user_stat select from_unixtime(floor(first_time/1000),'yyyy-MM-dd') as stat_day,count(DISTINCT CELL_PHONE_DEVICE_ID ) as new_user_count from sdk_user_info group by from_unixtime(floor(first_time/1000),'yyyy-MM-dd');
