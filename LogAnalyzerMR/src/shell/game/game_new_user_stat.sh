...
hive -e "
    ...
    Load Data Inpath '${outputPath}/part-r-*' Overwrite Into Table game_user_info;
    ...
    insert overwrite table game_new_user_stat select from_unixtime(floor(first_time/1000),'yyyy-MM-dd') as stat_day,PACKAGE_NAME,VERSION_CODE,count(DISTINCT CELL_PHONE_DEVICE_ID ) as user_count from game_user_info group by from_unixtime(floor(first_time/1000),'yyyy-MM-dd'),PACKAGE_NAME,VERSION_CODE;
